<template>
  ...
  >
    ...
    >
      ...
      Filtro
      <img
        src="@img/icon/icon-filter.svg"
        :class="{ 'icon-count': count || showFilters }"
      />
    </span>

    <div :class="[dropwnContentClass, { open: showFilters }]">
      <dl v-if="isMobile" class="filter-menu">
        <template v-for="(item, index) in filters" :key="`title-${index}`">
          <dt
            :class="[
              'item',
              item.type,
  ...
  methods: {
    ...
    handleChangeFilterModel(value) {
      ...
      this.filtersSelected = {
        ...this.filtersSelected,
        [this.activeFilter.name]: isDOMEvent ? value.target.value : value,
      };
    },
    toggleShowFilters() {
